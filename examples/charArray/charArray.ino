...
// Выводим информацию о переменных в Монитор порта
  PRINT_INFO_ARRAY(SSIDwifi);
  PRINT_INFO_ARRAY(PASSwifi);
...
// Выводим код и результат его работы в Монитор порта для наглядности
  strcpy(SSIDwifi.v, "ESP8266");  Serial.print("strcpy(SSIDwifi.v, ESP8266); -> \t");     PRINT_VALUE_ARRAY(SSIDwifi);
  SSIDwifi.Save();                Serial.print("SSIDwifi.Save();             -> \t");     PRINT_VALUE_ARRAY(SSIDwifi);
  strcpy(SSIDwifi.v, "ESP8266");  Serial.print("strcpy(SSIDwifi.v, ESP8266); -> \t");     PRINT_VALUE_ARRAY(SSIDwifi);
  strcat(SSIDwifi.v, "_My");      Serial.print("strcat(SSIDwifi.v, _My);     -> \t");     PRINT_VALUE_ARRAY(SSIDwifi);
  strcat(SSIDwifi.v, "WiFi");     Serial.print("strcat(SSIDwifi.v, WiFi);    -> \t");     PRINT_VALUE_ARRAY(SSIDwifi);
  SSIDwifi.Load();                Serial.print("SSIDwifi.Load();             -> \t");     PRINT_VALUE_ARRAY(SSIDwifi);
...
// А это значит что в памяти переменной SSIDwifi => ESP8266
